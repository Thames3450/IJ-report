...
export default function Reports({ machines, defects }) {
  ...
  const exportCSV = () => {
    const headers = ['ID','Asset Code','Asset Name','Type','Zone','Component','Problem','Location','Priority','Required Stop (min)','Repair Time (min)','Parts','Status','Created','Completed','Recommended Action','Corrective Action','Result','Completed By','Before Photo','After Photo']
    const rows = filtered.map((d) => {
      const m = machineMap[d.machine]
      return [d.id,d.machine,m?.label || d.machine,m?.type || '',m?.zone || '',d.component,d.problem,d.location,d.priority,d.stop,d.repair,d.parts,d.status,d.created,d.completed||'',d.action,d.resolutionAction||'',d.resolutionResult||'',d.completedBy||'',d.beforePhoto||'',d.afterPhoto||'']
    })
  ...
  return <>
    ...
    <div className="report-sheet" id="report-sheet">
      ...
      {evidenceDone.length > 0 && <div className="report-block report-evidence-block"><h3>Completed Work Evidence <Th>หลักฐานงานที่ปิดแล้ว Before / After</Th></h3><div className="report-evidence-grid">{evidenceDone.map((d) => <div className="report-evidence-item" key={d.id}><div className="report-evidence-head"><div><b>{machineMap[d.machine]?.label || d.machine} · {d.problem}</b><small>{d.id} · {d.component} · {d.location}</small></div><Pill tone="ok">Completed</Pill></div><div className="report-evidence-images"><div><span>Before</span>{d.beforePhoto ? <img src={d.beforePhoto} alt="Before" /> : <div className="report-photo-missing">No photo</div>}</div><div><span>After</span>{d.afterPhoto ? <img src={d.afterPhoto} alt="After" /> : <div className="report-photo-missing">No photo</div>}</div></div><p><b>Action:</b> {d.resolutionAction || '-'}</p><p><b>Result:</b> {d.resolutionResult || '-'}</p>{d.completedBy && <small>Completed by: {d.completedBy}</small>}</div>)}</div></div>}
